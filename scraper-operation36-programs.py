...
def parse_student_name(row_text):
    ...
    the leading 'student N' marker and trimming at 'Age'."""
    if not row_text:
        return ""
    s = row_text.strip()
...
async def collect_program_students(page, program_id, worker_id):
    """Visit /programs/{id}/roster and capture enrolled students."""
    ...
    print(f"  [w{worker_id}]   Found {count} student row(s)")
    ...
    for i in range(count):
        ...
        print(f"  [w{worker_id}]   {student_name} ({student_id}) -> {student_url}")
